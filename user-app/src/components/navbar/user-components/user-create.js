...
import { useRouter } from "next/navigation"  //linking de hien thi create len page
//nho next/navigation
...
export default function CreateUserForm({ createUserAction }) {
    ...
    const handleCreateUser = async (values) => {
        ...
        console.log('client log:', result)
        if(Object.keys(result).length){
            toast.success(`User ${result.username} created successfully`)
            router.push('/users')
        }
    }
    return (
            ...
            <form onSubmit={handleSubmit(handleCreateUser)} className="w-50 border rounded p-2">
                ...
                <div className="form-group mb-2">
                    ...
                    <button type="button" className="btn btn-dark me-2"
                        onClick={() => reset()} //click cancel xoa messaging
                    >Cancel</button>
                </div>
            </form>
        </>
